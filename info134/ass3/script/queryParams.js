...
function get_query_string_parameters() {
    ...
    if (uri.indexOf("?") < 0)
        return result;
    let query_string = uri.substring(uri.indexOf("?"));
    if (query_string.indexOf("#") >= 0)
        query_string = query_string.substring(0, query_string.indexOf("#"));

    
    let query_pattern = /(?:[?&])([^&=]+)(?:=)([^&]*)/g;
    let decode = function(str) {return decodeURIComponent(str.replace(/\+/g, " "));};
    //Produces warning, but syntax is ok and everything should be ok.
    let match;
